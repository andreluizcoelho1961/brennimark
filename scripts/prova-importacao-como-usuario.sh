#!/usr/bin/env bash
#
# Roda a prova da importação feita como usuário comum (defeito do ensaio de 18/09).
#
...
docker exec -i "$CONTAINER" psql -U postgres -q -f - < "$AQUI/prova-importacao-como-usuario.sql"
