...
type Version = {
  id: string;
  action: "published" | "restored_to_matrix";
  actorLabel: string;
  createdAt: string;
  title: string;
  status: DocStatus;
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (response.ok) {
        ...
        setMatrixActive(Boolean(data.matrixActive));
      } else {
  ...
  return (
    <section className="mt-10 border-t border-border-default pt-8" aria-labelledby="version-history-title">
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        {matrixActive && <span className="border border-release-analog-turquoise px-3 py-1 font-display text-[10px] font-bold uppercase text-release-analog-turquoise">{isEnglish ? "Using original matrix" : "Matriz original em uso"}</span>}
      </div>
      <p className="mt-3 max-w-2xl text-sm leading-relaxed text-text-secondary">{isEnglish ? "Every publish and restore is logged automatically. History can't be edited or deleted from the interface." : "Cada publicação e restauração é registrada automaticamente. O histórico não pode ser editado ou apagado pela interface."}</p>
      ...
      {!loading && versions.length === 0 && !message && <p className="mt-6 border border-dashed border-border-default p-5 text-sm text-text-secondary">{isEnglish ? "No custom versions of this page yet. The first save will start the timeline." : "Ainda não há versões personalizadas desta página. O primeiro salvamento iniciará a linha do tempo."}</p>}
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <div className="flex flex-wrap items-center gap-2">
                <span className={`px-2 py-1 font-display text-[9px] font-black uppercase ${version.action === "restored_to_matrix" ? "border border-border-default text-text-secondary" : "bg-release-analog-turquoise text-release-analog-black"}`}>
                  {version.action === "restored_to_matrix" ? (isEnglish ? "Matrix restored" : "Matriz restaurada") : (isEnglish ? "Published" : "Publicada")}
                </span>
