...
export default function AnalysisHistoryPage() {
  ...
  return (
    <article className="px-page-inline py-12 md:py-16">
      <div className="inline-flex bg-platform-signal px-4 py-1.5">
        <span className="font-display text-[11px] font-black uppercase tracking-[0.2em] text-platform-bg">{isEnglish ? "Governance" : "Governança"}</span>
      </div>
      <div className="mt-6 flex flex-wrap items-end justify-between gap-5">
        <div>
          <h1 className="font-display font-black uppercase leading-[0.9] tracking-tight text-platform-text" style={{ fontSize: "clamp(2rem, 4.5vw, 4rem)" }}>
            {isEnglish ? <>History &amp; <span className="text-platform-text">Calibration</span></> : <>Histórico e <span className="text-platform-text">Calibração</span></>}
          </h1>
          <p className="mt-4 max-w-3xl text-sm leading-relaxed text-platform-text-muted">
            {isEnglish
        ...
        <Link href={`${base}/analise`} className="bg-platform-signal px-5 py-3 font-display text-xs font-bold uppercase tracking-wide text-platform-bg">
          {isEnglish ? "New analysis" : "Nova análise"}
        </Link>
      </div>

      {loading && <p className="mt-10 text-sm text-platform-text-muted" role="status">{isEnglish ? "Loading history…" : "Carregando histórico…"}</p>}
      {error && <p className="mt-10 border-l-2 border-platform-border pl-4 text-sm text-platform-text-muted" role="alert">{error}</p>}

      {!loading && !error && (
        <>
          <section className="mt-10 grid gap-px bg-platform-border sm:grid-cols-2 xl:grid-cols-5">
            {(isEnglish
              ? [
              ...
              <div key={label} className="bg-platform-panel p-5">
                <p className="font-display text-[10px] font-bold uppercase tracking-wide text-platform-text-muted">{label}</p>
                <p className="mt-2 font-display text-2xl font-black text-platform-text">{value}</p>
              </div>
            ))}
          </section>

          <section className="mt-10 grid gap-6 lg:grid-cols-2">
            <div className="border border-platform-border p-6">
              <h2 className="font-display text-sm font-black uppercase tracking-wide text-platform-text">{isEnglish ? "Recurring issues" : "Problemas recorrentes"}</h2>
              {patterns.length ? (
                <ol className="mt-5 space-y-4">
                  {patterns.map((pattern, index) => (
                    <li key={`${pattern.label}-${index}`} className="grid grid-cols-[2rem_1fr_auto] gap-3 text-sm">
                      <span className="font-mono text-platform-text">{String(index + 1).padStart(2, "0")}</span>
                      <span className="leading-relaxed text-platform-text">{pattern.label}</span>
                      <span className="font-mono text-xs text-platform-text-muted">{pattern.count}×</span>
                    </li>
                  ))}
                </ol>
              ) : <p className="mt-5 text-sm text-platform-text-muted">{isEnglish ? "Patterns will appear after the first analyses." : "Os padrões aparecerão após as primeiras análises."}</p>}
            </div>
            <div className="border border-platform-border p-6">
              <h2 className="font-display text-sm font-black uppercase tracking-wide text-platform-text">{isEnglish ? "Possible gaps in the guide" : "Possíveis lacunas do guide"}</h2>
              {gaps.length ? (
                <ul className="mt-5 space-y-3 text-sm leading-relaxed text-platform-text">
                  {gaps.map((gap, index) => <li key={`${gap}-${index}`} className="border-l border-platform-border pl-4">{gap}</li>)}
                </ul>
              ) : <p className="mt-5 text-sm leading-relaxed text-platform-text-muted">{isEnglish ? "No explicit gap was detected. This panel looks for answers with insufficient, ambiguous, or undocumented information." : "Nenhuma lacuna explícita foi detectada. O painel procura respostas com informação insuficiente, ambígua ou não documentada."}</p>}
            </div>
          </section>

          <section className="mt-12">
            <div className="flex flex-wrap items-end justify-between gap-4 border-b border-platform-border pb-4">
              <div>
                <h2 className="font-display text-xl font-black uppercase text-platform-text">{isEnglish ? "All analyses" : "Todas as análises"}</h2>
              ...
              <div className="flex flex-wrap gap-2">
                <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={isEnglish ? "Search piece or question" : "Buscar peça ou pergunta"} aria-label={isEnglish ? "Search history" : "Buscar no histórico"} className="min-w-56 border border-platform-border bg-transparent px-3 py-2 text-sm text-platform-text placeholder:text-platform-text-muted focus:border-platform-signal focus:outline-none" />
                <select value={verdict} onChange={(event) => setVerdict(event.target.value as AnalysisVerdict | "all")} aria-label={isEnglish ? "Filter by verdict" : "Filtrar por veredito"} className="border border-platform-border bg-platform-bg px-3 py-2 text-sm text-platform-text">
            ...
            {filtered.length ? (
              <div className="mt-6 grid gap-4 xl:grid-cols-2">
                {filtered.map((run) => (
                  <Link key={run.id} href={`${base}/historico/${run.id}`} className="group grid min-h-40 grid-cols-[7rem_1fr] overflow-hidden border border-platform-border bg-platform-panel transition-colors hover:border-platform-signal">
                    <div className="bg-platform-panel">
                      {run.imageUrl ? (
                        // eslint-disable-next-line @next/next/no-img-element
                        <img src={run.imageUrl} alt="" className="h-full w-full object-cover" />
                      ) : <div className="grid h-full place-items-center font-mono text-xs text-platform-text-muted">{isEnglish ? "NO IMG" : "SEM IMG"}</div>}
                    </div>
                    <div className="p-4">
                      <div className="flex items-start justify-between gap-3">
                        <p className="line-clamp-2 font-display text-sm font-bold uppercase text-platform-text">{run.fileName}</p>
                        {run.calibrationEnabled && <span className="border border-platform-signal px-2 py-1 font-display text-[8px] font-bold uppercase text-platform-text">{isEnglish ? "Reference" : "Referência"}</span>}
                      </div>
                      <p className="mt-2 line-clamp-2 text-xs leading-relaxed text-platform-text-muted">{run.question}</p>
                      <div className="mt-4 flex flex-wrap items-center gap-x-4 gap-y-2 text-[10px] uppercase tracking-wide text-platform-text-muted">
                        <span className="text-platform-text">{VERDICT_LABELS[run.verdict]}</span>
            ...
            ) : <p className="mt-8 text-sm text-platform-text-muted">{isEnglish ? "No analysis matches the filters." : "Nenhuma análise corresponde aos filtros."}</p>}
          </section>
        </>
      )}
    </article>
